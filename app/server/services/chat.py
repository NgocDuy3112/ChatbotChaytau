...
from ..utils.file_utils import upload_file_to_gemini, get_file_hash, extract_docx_text, extract_xlsx_text
        ...
        if suffix in {".docx", ".xlsx"}:
            ...
            try:
                if suffix == ".docx":
                    ...
                    extracted_text = extract_xlsx_text(p)
            except Exception as exc:
                global_logger.warning(
                    f"{suffix.upper()} extraction failed for {p.name}, fallback to file upload: {exc}"
                )

            if extracted_text:
                text_part = types.Part(
                    text=(
                        f"Extracted content from {p.name}. Use this as authoritative file context:\n"
                        f"{extracted_text[:100000]}"
                    )
                )
